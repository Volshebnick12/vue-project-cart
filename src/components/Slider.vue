<template>
    <div class="slider">
        <div class="slider__top">
            ...
            <p class="slider__top-number">{{ pagination }} <span class="slider__top-number-list">/ {{ paginationAll }}</span></p>
        </div>
        <div class="slider__cards">
            <div class="slider__cards-wrapper" :style="{ 'margin-left': '-' + (365 * currentSlideIndex) + 'px' }">
                <CardMini v-for="card in this.$store.state.products" :key="card.index" :product_data="card"
                    class="slider__cards-wrapper-card" />
            </div>
